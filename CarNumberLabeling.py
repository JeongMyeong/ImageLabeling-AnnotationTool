...
class Labeling(Frame):
    ...
    def click(self,event):  # 클릭했을때 커맨드
        ...
        if(len(self.xy[self.car_cnt-1])<7):
            self.xy[self.car_cnt-1].append(self.canvas.canvasx(event.x))
            self.xy[self.car_cnt-1].append(self.canvas.canvasy(event.y))
    ...
    def MsgMotion(self,event):                  #사진위에서 마우스 올라갔을때 커맨드
        ...
        self.xytext.config(text=str(self.x) + ',' + str(self.y))
        # self.x = event.x
        # self.y = event.y
        self.draw()
        sel_car = -1
    ...
    def viewImage(self):  # 이미지를 띄우는 명령
        ...
        self.dataLoad()
        self.canvas = Canvas(width=self.user_width,height=self.user_height,scrollregion=(0, 0, 1000, 1000))                                             #없는파일일때 처리 해야함
